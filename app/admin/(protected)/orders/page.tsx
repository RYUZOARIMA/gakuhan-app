...
type SchoolWithOrders = {
  ...
  orders: (OrderRow & { items: CreatedOrderItem[]; total: number })[];
};

async function loadOrdersBySchool(): Promise<SchoolWithOrders[]> {
  const schools = await listSchools();

  return Promise.all(
    schools.map(async (school) => {
      const orders = await listOrders(school.id);
      ...
      return { school, orders: ordersWithItems };
    }),
  );
}

export default async function AdminOrdersPage() {
  const schoolsWithOrders = await loadOrdersBySchool();
